...
void find_affinity_groups(){
  ...
  std::vector<shortpair> all_affEntry_iters;
  ...
  for(std::vector<shortpair>::iterator iter=all_affEntry_iters.begin(); iter!=all_affEntry_iters.end(); ++iter){
...
void * update_affinity(void * threadno_void){
  ...
  while(true){
    ...
    while(trace_list_queue.pop(trace_list_front)){
      ...
      freqs[threadno][FuncNum][top_window_iter->wsize]++;
    ...
    bool affEntry1DCmp(shortpair left_pair, shortpair right_pair){

    int * jointFreq_left = (*sum_affEntries)[left_pair];
    int * jointFreq_right = (*sum_affEntries)[right_pair];
    ...
    if(jointFreq_left != NULL){
      ...
      for(short wsize=2;wsize<=maxWindowSize;++wsize){
        ...
        if((rel_freq_threshold*(jointFreq_left[wsize]) >= sum_freqs[left_pair.first][wsize]) && 
            (rel_freq_threshold*(jointFreq_left[wsize]) >= sum_freqs[left_pair.second][wsize]))
          left_pair_val = 1;
        else
          left_pair_val = -1;

        if((rel_freq_threshold*(jointFreq_right[wsize]) >= sum_freqs[right_pair.first][wsize]) && 
            (rel_freq_threshold*(jointFreq_right[wsize]) >= sum_freqs[right_pair.second][wsize]))
          right_pair_val = 1;
        else
          right_pair_val = -1;
  ...
  bool affEntry2DCmp(shortpair left_pair, shortpair right_pair){
    ...
    int * jointFreq_left_rev = (*sum_affEntries)[left_pair_rev];
    int * jointFreq_right_rev = (*sum_affEntries)[right_pair_rev];
    ...
    if(jointFreq_left != NULL){
      ...
      for(freqlevel=0, rel_freq_threshold=1.0; freqlevel<maxFreqLevel; ++freqlevel, rel_freq_threshold+=5.0/maxFreqLevel){
        for(short wsize=2;wsize<=maxWindowSize;++wsize){
          ...
          if((rel_freq_threshold*(jointFreq_left[wsize]) >= sum_freqs[left_pair.first][wsize]) && 
              (rel_freq_threshold*(jointFreq_left_rev[wsize]) >= sum_freqs[left_pair_rev.first][wsize]))
            left_pair_val = 1;
          else
            left_pair_val = -1;

          if((rel_freq_threshold*(jointFreq_right[wsize]) >= sum_freqs[right_pair.first][wsize]) && 
              (rel_freq_threshold*(jointFreq_right_rev[wsize]) >= sum_freqs[right_pair_rev.first][wsize]))
            right_pair_val = 1;
          else
            right_pair_val = -1;
