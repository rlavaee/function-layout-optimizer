...
void aggregate_affinity(){
		...
		if(graphFile!=NULL){
				...
				for(uint32_t i=0; i<fall_through_pairs; ++i){
						fscanf(graphFile,"%hu[%hu,%hu]:%u",&fid,&bbid1,&bbid2,&fallt);
...
void emit_graphFile(){
		...
		for(siter=single_freqs.begin(); siter!=single_freqs.end(); ++siter){
				fprintf(graphFile,"%u:",siter->first);
				for(wsize_t wsize=0; wsize<maxWindowSize;++wsize)
		...
		for(jiter=joint_freqs.begin(); jiter!=joint_freqs.end(); ++jiter){
				fprintf(graphFile,"[%u,%u]:",jiter->first.first,jiter->first.second);
				for(wsize_t wsize=0;wsize<maxWindowSize;++wsize)
...
void find_affinity_groups(){
		...
		for(func_t fid=0; fid < totalFuncs; ++fid){
				...
				order_out << "fallthrough bbs for fid: " << fid << endl;
				cur_fid = fid;
...
void disjointSet::mergeSets(Block left_rec, Block right_rec){
		...
		//cerr << "merging left_set: " << left_set << "\t and right_set: " << right_set << endl;

		if(left_set==right_set)
				return;

		//if(left_rec.fid!=right_rec.fid)
		//	order_out << "Merging" << endl;
