...
void find_affinity_groups(){
		...
		for(func_t fid=0; fid < totalFuncs; ++fid){
			...
			*/
			...
			//cerr << "This cfg has " << bb_count[fid] << " bbs!" << endl;
				...
				//to get the atomic functions
				for(bb_t bbid=0; bbid<bb_count[fid]-1; ++bbid)
							disjointSet::mergeSets(fid << 16 | bbid,
													fid << 16 | (bbid+1),
														true);;
		...
		for(func_t fid=0; fid < totalFuncs-1; ++fid)
			disjointSet::mergeSets(fid<<16, (fid+1) << 16, true);
...
void disjointSet::mergeSets(Block left_rec, Block right_rec, bool force_merge){
		...
		if((rec_dist_min > MAX_DIST) && !force_merge)
			return;
